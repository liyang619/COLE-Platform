...
then
    ...
    elif [ "$1" = "god_game_shapley" ]
    then
        tensorflowjs_converter --input_format=tf_saved_model --output_node_names='ego_agent/ppo2_model/action_probs' --saved_model_tags=serve human_aware_rl/data/$RUN_TYPE/$RUN_NAME/seed_$SEED/ego_agent/best \
            human_aware_rl/data/web_models/$RUN_NAME\_seed$SEED\_temp
    else
