...
def save_variables(save_path, variables=None, sess=None):
    ...
    # jichen edit 
    # save_dict = {v.name: value for v, value in zip(variables, ps) if 'agent0' in v.name}
...
def load_variables(load_path, variables=None, sess=None, agent_name="", agent_suffix="", scope=None):
    import joblib
    sess = sess or get_session()
    variables = variables or tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)

    loaded_params = joblib.load(os.path.expanduser(load_path))
    # logger.info(f"varibles: {variables}")
    # logger.info(f"loaded_params: {loaded_params}")
    restores = []
    if isinstance(loaded_params, list):
        ...
        # jichen edit this to make loading multiple agents with same name work
        if not scope:
            for v in variables:
                v_name = v.name
                ...
                if agent_suffix:
                    if agent_name not in v_name or agent_suffix not in v_name:
                        continue
                    v_name = v.name.replace(agent_suffix, "")
                # logger.info(f'{v_name}')
                    # print(f"Old_name is {v.name}, new name is {v_name}")
                restores.append(v.assign(loaded_params[v_name]))
        else:
            ori_pre = list(loaded_params.keys())[0].split('/')[0]
            for v in variables:
