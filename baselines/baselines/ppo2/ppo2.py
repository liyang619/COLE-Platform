...
def shuffle(obs, returns, masks, actions, values, neglogpacs):
    #################
    # Added by Yang Li
    # Aim to
    # shuffle data
    #################
    index = np.random.permutation(obs.shape[0])

    return obs[index], returns[index], masks[index], actions[index], values[index], neglogpacs[index]


def learn(*, network, env, total_timesteps, early_stopping=False, eval_env=None, seed=None, nsteps=2048,
          ent_coef=0.0, ent_pool_coef=0.0, lr=3e-4, vf_coef=0.5, max_grad_norm=0.5, gamma=0.99,
          lam=0.95, log_interval=10, other_partners=None, nminibatches=4, noptepochs=4, cliprange=0.2,
          save_interval=0, load_path=None,
          model_fn=None, population=None, scope='', **network_kwargs):
    '''
    ...
    log_interval: int                 number of timesteps between logging

    other_partners: agent list
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    if other_partners is not None:
        real_nsteps = int(nsteps / (len(other_partners)))
        nbatch = nenvs * nsteps
    ...
    model = model_fn(policy=policy, ob_space=ob_space, ac_space=ac_space, nbatch_act=nenvs, nbatch_train=nbatch_train,
                     nsteps=real_nsteps, ent_coef=ent_coef, vf_coef=vf_coef,
                     max_grad_norm=max_grad_norm, scope=scope, load_path=load_path)

    if load_path is not None:
        model.load(load_path)
    # Instantiate the runner object

    # env.venv.remote_set_agent_idx(0)
    if other_partners == None:
        runner = Runner(env=env, model=model, nsteps=real_nsteps, gamma=gamma, lam=lam, population=population,
                        ent_pool_coef=ent_pool_coef)
    else:
        runner = []
        for other_env in other_partners:
            runner += [Runner(env=env, model=model, nsteps=real_nsteps, gamma=gamma, lam=lam, others=other_env)]
    if eval_env is not None:
        # env.venv.remote_set_agent_idx(0)
        eval_runner = Runner(env=eval_env, model=model, nsteps=real_nsteps, gamma=gamma, lam=lam)
    ...
    print("TOT NUM UPDATES {}={}/{}".format(nupdates, total_timesteps, nbatch))

    log_milestone = int(nupdates / 10)
    if log_milestone == 0:
        log_milestone = 1
    for update in range(1, nupdates + 1):
        ...
        if other_partners == None:
            ...
            obs, returns, masks, actions, values, neglogpacs, states, epinfos = runner[0].run()  # pylint: disable=E0632
            # print("main runner agent's idx {}".format(runner[0].env.venv.remote_get_agent_idx()))
            # print("main runner agent  {}".format(runner[0].env.other_agent))
            for other_runner in runner[1:]:
                # if update==1:
                neg_obs, neg_returns, neg_masks, neg_actions, neg_values, neg_neglogpacs, neg_states, neg_epinfos = other_runner.run()  # pylint: disable=E0632

                # print("other runner agent's idx {}".format(other_runner.env.venv.remote_get_agent_idx()))
                # print("other runner agent  {}".format(other_runner.env.other_agent))
                obs = cat(obs, neg_obs)
                returns = cat(returns, neg_returns)
                masks = cat(masks, neg_masks)
                actions = cat(actions, neg_actions)
                values = cat(values, neg_values)
                neglogpacs = cat(neglogpacs, neg_neglogpacs)
                if states is not None:
                    states = cat(states, neg_states)
                epinfos = cat(epinfos, neg_epinfos)
        ...
        if update % log_milestone == 0 or update == 1:
        ...
        obs, returns, masks, actions, values, neglogpacs = shuffle(obs, returns, masks, actions, values, neglogpacs)
        ...
        if update % log_interval == 0 or update == 1:
            ...
            logger.logkv("ent_pool_coef", log_runner.ent_pool_coef)
            logger.logkv("entropy_pop_delta", log_runner.entropy_pop_delta_mean)
            logger.logkv("entropy_pop_new", log_runner.entropy_pop_new_mean)
            logger.logkv("neg_logp_pop_new", log_runner.neg_logp_pop_new_mean)
            logger.logkv("neg_logp_pop_delta", log_runner.neg_logp_pop_delta_mean)
            ...
            run_info['ent_pool_coef'].append(log_runner.ent_pool_coef)
            run_info['entropy_pop_delta'].append(log_runner.entropy_pop_delta_mean)
            run_info['entropy_pop_new'].append(log_runner.entropy_pop_new_mean)
            run_info['neg_logp_pop_new'].append(log_runner.neg_logp_pop_new_mean)
            run_info['neg_logp_pop_delta'].append(log_runner.neg_logp_pop_delta_mean)
