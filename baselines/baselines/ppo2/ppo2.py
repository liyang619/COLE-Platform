...
def learn(*, network, env, total_timesteps, early_stopping = False, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4,
            ...
            save_interval=0, load_path=None, model_fn=None, scope='', 
            ent_pool_coef=0.0, ent_version=1, 
            population=None, metric_np=None, 
            agent_name="", agent_suffix="",
            other_partners=None, **network_kwargs):
    '''
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    model = model_fn(policy=policy, ob_space=ob_space, ac_space=ac_space, nbatch_act=nenvs, nbatch_train=nbatch_train,
                    ...
                    max_grad_norm=max_grad_norm, scope=agent_name+agent_suffix)

    # jichen edit for multiple agent loading
    if load_path is not None:
        model.load(load_path, agent_name=agent_name, agent_suffix=agent_suffix, scope=scope)
    ...
    for update in range(1, nupdates+1):
            ...
            if additional_params["RUN_TYPE"] in ["ppo", "joint_ppo"]:
                ...
                if ep_sparse_rew_mean > bestrew and ep_sparse_rew_mean > additional_params["SAVE_BEST_THRESH"]:
                    ...
                    if additional_params["OTHER_AGENT_TYPE"][:2] == "bc" and sp_horizon != 0 and env.self_play_randomization > 0:
                        ...
                        from human_aware_rl.baselines_utils import save_ppo_model_full
                        print("BEST REW", ep_sparse_rew_mean, "overwriting previous model with", bestrew)

                        run_info['agent_name'] = model.agent_name
                        save_ppo_model_full(f'{additional_params["SAVE_DIR"]}seed{additional_params["CURR_SEED"]}/best',
                                model,
                                run_info,
                                additional_params
                                )
        ...
        print("get_dir:", logger.get_dir())
        print("MPI:", (MPI is None or MPI.COMM_WORLD.Get_rank() == 0))        
        ...
        if save_interval and (update % save_interval == 0 or update == 1) and logger.get_dir() and (MPI is None or MPI.COMM_WORLD.Get_rank() == 0):
            ...
            print("-"*20)
            print(f"saving checkpoint {update}, ep_sparse_rew_mean: {ep_sparse_rew_mean}")
            print("-"*20)
            
            # must use different name otherwise will fail in loading multiple agents
            run_info['agent_name'] = model.agent_name
            # should use save_baselines_model
            save_ppo_model_full(f'{additional_params["SAVE_DIR"]}seed{additional_params["CURR_SEED"]}/{update}_reward={ep_sparse_rew_mean:.2f}',
                                model,
                                run_info,
                                additional_params
                                )
        ...
        if run_type in ["ppo", "joint_ppo"] and update % additional_params["VIZ_FREQUENCY"] == 0:
            ...
            # additional_params["selfish"] = True
