...
class Runner(AbstractEnvRunner):
    ...
    - Make a mini batch
    """
    def __init__(self, *, env, model, nsteps, gamma, lam, population=None, ent_pool_coef=0.0, ent_version=3, history_len=10, others=None):
            ...
            if overcooked:
                if self.population is not None:
                    ...
                    for agent in self.population:
                        # print(agent.model.step)
                        actions, _, _, _, action_probs = agent.model.step(self.obs0, return_full=True, S=self.states, M=self.dones)
                ...
                actions, values, self.states, neglogpacs, action_probs_agent0 = self.model.step(self.obs0, return_full=True, S=self.states, M=self.dones)
