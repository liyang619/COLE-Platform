...
class ActorCriticRLModel(BaseRLModel):
    ...
    @classmethod
    def load_part(cls, load_path, env=None, **kwargs):
        data, params = cls._load_from_file(load_path)
        ...
        restores = []
        for param, loaded_p in zip(model.params, params):
            if "pi_fc1" not in param.name:
                restores.append(param.assign(loaded_p))
            else:
                print("we dont load {}'s param.".format(param.name))
        model.sess.run(restores)

        return model

    @classmethod
    def load_none(cls, load_path, env=None, **kwargs):
        data, params = cls._load_from_file(load_path)
        ...
        restores = []
        model.sess.run(restores)
        print('It is initialized model. we dont load anything.')
