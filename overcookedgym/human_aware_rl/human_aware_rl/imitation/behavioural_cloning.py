...
def get_bc_agent_from_saved_compat(model_name, no_waits=False):
    model, bc_params = load_bc_model_from_path(model_name)
    state_policy, processed_obs_policy = get_bc_model_policy(model, bc_params, no_waits)
    return AgentFromPolicy(state_policy, processed_obs_policy), bc_params

def get_bc_model_policy(model, bc_params, no_waits=False):
    mdp = OvercookedGridworld.from_layout_name(**bc_params["mdp_params"])
    mlp = MediumLevelPlanner.from_pickle_or_compute(mdp, NO_COUNTERS_PARAMS, force_compute=False)
