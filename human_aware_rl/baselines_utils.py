...
def conv_network_fn(**kwargs):
    ...
    def network_fn(X):
        ...
        logger.info("Last layer conv network output shape", out.shape)
...
def save_ppo_model_full(save_folder, model, logs, params):
        """Save agent model, logs, and parameters"""
        logger.info(f"------ saving to {save_folder} -------")
        delete_dir_if_exists(save_folder)
        ...
        save_dict_to_file(dict(logs), os.path.join(save_folder,"logs"))
        save_dict_to_file(params, os.path.join(save_folder,"params"))
...
def load_baselines_model(save_dir, agent_name, config, agent_suffix="", scope=""):
    """
    # jichen edit this to make loading multiple agents
...
def load_trained_model(save_dir, agent_name, config):
    """
    NOTE: Before using load it might be necessary to clear the tensorflow graph
    if there are already other variables defined
    """
    ...
    model, _ = learn(
        ...
        nminibatches=config["MINIBATCHES"],
        noptepochs=config["STEPS_PER_UPDATE"],
        load_path=save_dir + "/model",
        scope=agent_name,
        network_kwargs=config
    )
...
def update_model(env, model, population=None, other_partners=None, ent_version=1, metric_np=None, **kwargs):
    """
    ...
    updated_model, run_info = learn(
        ...
        ent_pool_coef=kwargs.get("ENTROPY_POOL", 0.0), 
        lr=kwargs["LR"], 
