...
def get_part_bc_agent_from_saved(model_name, no_waits=False):
    model, bc_params = load_part_bc_model_from_path(model_name)
    return get_bc_agent_from_model(model, bc_params, no_waits), bc_params

def get_none_bc_agent_from_saved(model_name, no_waits=False):
    model, bc_params = load_none_bc_model_from_path(model_name)
    return get_bc_agent_from_model(model, bc_params, no_waits), bc_params
...
def load_part_bc_model_from_path(model_name):
    # NOTE: The lowest loss and highest accuracy models
    # were also saved, can be found in the same dir with
    # special suffixes.
    bc_metadata = load_pickle(BC_SAVE_DIR + model_name + "/bc_metadata")
    bc_params = bc_metadata["bc_params"]
    model = GAIL.load_part(BC_SAVE_DIR + model_name + "/model")
    return model, bc_params

def load_none_bc_model_from_path(model_name):
    # NOTE: The lowest loss and highest accuracy models
    # were also saved, can be found in the same dir with
    # special suffixes.
    bc_metadata = load_pickle(BC_SAVE_DIR + model_name + "/bc_metadata")
    bc_params = bc_metadata["bc_params"]
    model = GAIL.load_none(BC_SAVE_DIR + model_name + "/model")
    return model, bc_params
