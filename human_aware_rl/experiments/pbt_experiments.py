...
from baselines_utils import get_pbt_agent_from_config
from behavioural_cloning import get_bc_agent_from_saved
...
if __name__ == "__main__":
    # layouts = ["simple", "random1", "random0", "random3", "unident_s", ]
    # pbt_model_paths = {
    #     "simple": "pbt_simple",
    #     "random1": "pbt_ra ndom1",
    #     "random3": "pbt_random3",
    #     "random0": "pbt_random0",
    #     "unident_s": "pbt_unident_s",
    # }
    # pbt_seeds = [8015, 3554,  581, 5608, 4221]
    # plot_pbt_runs(pbt_model_paths, pbt_seeds, show=True, single=False, save=True)
    load = True
    if load:
        from overcooked_ai_py.utils import save_dict_to_file
        pbt_performance = load_pickle(PBT_DATA_DIR + "pbt_performance")
        save_dict_to_file(pbt_performance, PBT_DATA_DIR + "pbt_performance")
    else:
        best_bc_models_paths = load_pickle("data/bc_runs/best_bc_model_paths.pickle")
        run_all_pbt_experiments(best_bc_models_paths)
