...
if __name__ == '__main__':
    ...
    num_rounds = args.num
    print(num_rounds)
    # 这里定义细分的路径（每个layout对应的目录名字，比如FCP输出的目录就在pbt_runs/FCP_xxx）
    pbt_model_paths = {
        # "simple": "pbt_simple",
    ...
    model_paths_seeds = {}
    # 下面用到了PBT_DATA_DIR，这个是data下的一级目录，FCP现在和PBT存在了同一个目录下，都是pbt_runs
    for key, value in pbt_model_paths.items():
        path = os.path.join("{}{}".format(PBT_DATA_DIR, value))
        seeds = []
        if os.path.isdir(path):
            # 这里代码直接读目录下文件的名字，然后找到所有的seed
            for seed in os.listdir(path):
                if os.path.isdir(os.path.join(path, seed)):
                    seeds += [int(seed.split('_')[1])]
        if len(seeds) != 0:
            model_paths_seeds[key] = ["pbt_{}".format(key), seeds]
    print(model_paths_seeds)
    best_bc_model_paths = load_pickle("./data/bc_runs/best_bc_model_paths") # 读取和behavior cloning评估的agent，不需要修改
