...
class OvercookedEnv(object):
    ...
    def step(self, joint_action):
        ...
        next_state, sparse_reward_all, reward_shaping_all = self.mdp.get_state_transition(self.state, joint_action)
        sparse_reward = sparse_reward_all[self.agent_idx]
        reward_shaping = reward_shaping_all[self.agent_idx]
        if self.mdp.selfish:
            self.cumulative_sparse_rewards += (sparse_reward_all[self.agent_idx] + sparse_reward_all[1 - self.agent_idx])
...
class Overcooked(gym.Env):
    ...
    ****
    ...
    def reset(self):
        ...
        self.base_env.agent_idx = self.agent_idx
