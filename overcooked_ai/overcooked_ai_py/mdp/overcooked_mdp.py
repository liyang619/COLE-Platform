...
class OvercookedGridworld(object):
    ...
    def __init__(self, terrain, start_player_positions, start_order_list=None, cook_time=20, num_items_for_soup=3, delivery_reward=20, rew_shaping_params=None, layout_name="unnamed_layout", selfish=False):
        """
    ...
    def from_layout_name(layout_name, **params_to_overwrite):
        ...
        if "selfish" in params_to_overwrite.keys():
            base_layout_params['selfish'] = params_to_overwrite['selfish']
        else:
            base_layout_params['selfish'] = False
    ...
    def get_state_transition(self, state, joint_action):
        ...
        for i in range(len(new_state.players)):
            shaped_reward[i] += self.step_environment_effects(new_state)
    ...
    def resolve_interacts(self, new_state, joint_action):
        ...
        sparse_reward = [0, 0]
        shaped_reward = [0, 0]
        for idx, (player, action) in enumerate(zip(new_state.players, joint_action)):
            ...
            if terrain_type == 'X':
                ...
                if len(nearly_ready_pots) > dishes_already and len(dishes_on_counters) == 0:
                    shaped_reward[idx] += self.reward_shaping_params["DISH_PICKUP_REWARD"]
                ...
                if player.get_object().name == 'dish' and new_state.has_object(i_pos):
                    ...
                    if num_items == self.num_items_for_soup and cook_time >= self.soup_cooking_time:
                        ...
                        player.set_object(new_state.remove_object(i_pos))
                        shaped_reward[idx] += self.reward_shaping_params["SOUP_PICKUP_REWARD"]
                    ...
                    if not new_state.has_object(i_pos):
                        ...
                        shaped_reward[idx] += self.reward_shaping_params["PLACEMENT_IN_POT_REW"]
                ...
                if obj.name == 'soup':
                    ...
                    sparse_reward[idx] += delivery_rew                        
                    ...
                    if new_state.order_list is not None and len(new_state.order_list) == 0:
                        break
        if not self.selfish:
            sp = sparse_reward[0] + sparse_reward[1]
            sh = shaped_reward[0] + shaped_reward[1]
            return [sp, sp], [sh, sh]
        else:
